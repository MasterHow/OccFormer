...
# quad ssc 平均值以rgb顺序给出 图片以cv2读入为bgr顺序 因此torgb把图片先转为rgb再norm保证正确性
img_norm_cfg = dict(
    mean=[123.971, 123.564, 164.785], std=[84.271, 88.170, 72.966], to_rgb=True)
# camera_used = ['left']
camera_used = ['images_unfold']
...
point_cloud_range = [-12.8, -12.8, -1.2, 12.8, 12.8, 2.0]
# occ_size = [256, 256, 32]
occ_size = [64, 64, 8]
...
# norm_cfg = dict(type='GN', num_groups=min(32, channels // 2), requires_grad=True)       # when bs=1
...
dataset_type = 'CustomQuadLssDataset'
data_root = r'F:\SSC_Datasets\QuadSSC'
ann_file = r'F:\SSC_Datasets\QuadSSC\sequences\2024-06-20-20-46-59\new_static_label'
# default root
...
train_pipeline = [
    ...
    dict(type='CreateDepthFromLiDAR', data_root=data_root, dataset='quad'),
    # dict(type='LoadSemKittiAnnotation', bda_aug_conf=bda_aug_conf,
    #         is_train=True, point_cloud_range=point_cloud_range),
    dict(type='LoadQuadAnnotation', bda_aug_conf=bda_aug_conf,
            is_train=True, point_cloud_range=point_cloud_range),
