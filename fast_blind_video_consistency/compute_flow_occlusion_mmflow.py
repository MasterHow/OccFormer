...
if __name__ == "__main__":
    ...
    bw_flow_dir = os.path.join(opts.flow_dir, "bw_flow")
    if not os.path.isdir(bw_flow_dir):
        os.makedirs(bw_flow_dir)
    ...
    bw_occ_dir = os.path.join(opts.flow_dir, "bw_occlusion")
    if not os.path.isdir(bw_occ_dir):
        os.makedirs(bw_occ_dir)
    ...
    bw_rgb_dir = os.path.join(opts.flow_dir, "bw_flow_rgb")
    if not os.path.isdir(bw_rgb_dir):
        os.makedirs(bw_rgb_dir)
    ...
    for t in range(0, 5*len(frame_list) - 5, 5):
        ...
        img1 = cv2.resize(img1, (1024, 1024), interpolation=cv2.INTER_NEAREST)
        img2 = cv2.resize(img2, (1024, 1024), interpolation=cv2.INTER_NEAREST)
        ...
        bw_occ = utils.detect_occlusion(fw_flow, bw_flow)
        ...
        # bw_flow
        output_flow_filename = os.path.join(bw_flow_dir, "%06d.flo" % t)
        if not os.path.exists(output_flow_filename):
            utils.save_flo(bw_flow, output_flow_filename)
        ...
        # bw_occ
        output_occ_filename = os.path.join(bw_occ_dir, "%06d.png" % t)
        if not os.path.exists(output_occ_filename):
            utils.save_img(bw_occ, output_occ_filename)
        ...
        # bw_flow_rgb
        output_filename = os.path.join(bw_rgb_dir, "%06d.png" % t)
        if not os.path.exists(output_filename):
            flow_rgb = utils.flow_to_rgb(bw_flow)
            utils.save_img(flow_rgb, output_filename)
